...
async function getBrandsByEquipmentType(typeId: string): Promise<Omit<EquipmentBrand, "created_at" | "updated_at" | "deleted_at">[]> {
	if (!typeId) return []
	...
	try {
		...
		const { data, error } = await supabase.from("equipment_brands").select("id, name, equipments!inner(type_id)").is("deleted_at", null).eq("equipments.type_id", typeId)
